...
export const signup = async (req,res) => {
    try {
        ...
        if (newUser) {
			...
			await newUser.save(); // lưu xuống db

			res.status(201).json({ // tạo thành công thông báo với thông tin bên dưới
				_id: newUser._id, // id có gạch ngang đầu dòng dùng để làm khóa cho thuộc tính Id.
...
export const updateUser = async (req, res) => {
        ...
        try {
            ...
            if(!user) return res.status(400).json({error: "you can't not update other user's profile"})
            ...
            // passwword should be null in response
            user.password = null;
...
export const getUserProfile = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json(user)// sau khi lay du lieuj xong tra ve data la user
    } catch (error) {
        res.status(500).json({ err: err.message });
...
export const getSuggestedUser = async (req,res) => {
    try {
        ...
        const filterUsers = users.filter(user => !userFollowByYou.following.includes(user._id));
